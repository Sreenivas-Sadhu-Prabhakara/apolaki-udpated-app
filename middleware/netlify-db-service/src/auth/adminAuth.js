/**
 * Admin auth middleware — merged into netlify-db-service.
 * Validates admin-scoped JWTs and enforces rate limits.
 */

import rateLimit from 'express-rate-limit';
import jwt from 'jsonwebtoken';
import { adminSessions } from '../adminDb.js';

const JWT_SECRET = process.env.JWT_SECRET || 'dev-secret';

// ─── Rate Limiters ────────────────────────────────────────────────────────────

export const adminStandardLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: 20,
...
export const adminStrictLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: 3,
